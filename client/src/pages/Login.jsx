...
function Login() {
  ...
  // @validation — Password strength rules (Sign Up): length, upper, lower, number, special
  ...
  // @validation — Login & Sign Up: name regex, email, password rules, age/phone/address, login fields
  const validateForm = () => {
    if (state === "Sign Up") {
  ...
  const onSubmitHandler = async (e) => {
    ...
    // @validation — Block submit until validateForm() passes
    ...
    try {
      ...
      const msg =
        ...
        (error.code === "ERR_NETWORK" ? "Cannot reach server. Is the backend running on port 4000?" : null) ||
        error.message ||
