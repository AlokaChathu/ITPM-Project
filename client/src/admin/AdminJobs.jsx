...
function AdminJobs() {
  ...
  const handleCreateJob = async (e) => {
    ...
    const trimmedDesc = newJob.description.trim();

    if (!trimmedTitle || !trimmedCompany || !trimmedDesc) {
      return toast.error("Please fill out all required fields.");
    }
    if (trimmedTitle.length < 3 || trimmedTitle.length > 100) {
      return toast.error("Job title must be between 3 and 100 characters.");
    }
    if (trimmedCompany.length < 2 || trimmedCompany.length > 100) {
      return toast.error("Company name must be between 2 and 100 characters.");
    }
    if (trimmedDesc.length < 20) {
      return toast.error("Job description is too short. Please provide at least 20 characters.");
    }

    const parsedTechStack = newJob.techStack.split(',').map(item => item.trim()).filter(Boolean);
    if (parsedTechStack.some(item => item.length > 25)) {
      return toast.error("A tech stack item is unusually long. Did you forget to separate them with commas?");
    }
    // --- VALIDATION END ---
  ...
  const handleUpdateJob = async (e) => {
    ...
    // --- VALIDATION START ---
    const trimmedTitle = editingJob.title.trim();
    const trimmedCompany = editingJob.company.trim();
    const trimmedDesc = editingJob.description.trim();
    ...
    // Handle tech stack which might come as a string (if edited) or an array (if untouched)
    const currentTechStack = typeof editingJob.techStack === 'string'
      ? editingJob.techStack
      : editingJob.techStack.join(', ');

    const parsedTechStack = currentTechStack.split(',').map(item => item.trim()).filter(Boolean);
